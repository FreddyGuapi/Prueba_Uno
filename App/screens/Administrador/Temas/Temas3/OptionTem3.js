...
const OptionTem3 = ({ navigation }) => {
    ...
    async function loadRTDAta() {
        const suscriber = firestore().collection('Capitulo3').onSnapshot(querySnapshot => {
            const temas = []
        ...
        Alert.alert(
            ...
            [
                ...
                },
                {
                    text: "Si",
                    onPress: () => {
                        firestore().collection('Capitulo3').doc(item.key).delete().then(() => {
                            Alert.alert(
                                "Eliminación Exitosa",
        ...
        if (titulo == '' || descripcion == '') {
            ...
            if (imagen != '') {
                const reference = storage().ref(
                    `/Capitulos/Capitulo3/imagenes/${editDoc}.jpg`,
                );
            ...
            if (QR != '') {
                const reference = storage().ref(
                    `/Capitulos/Capitulo3/QR/${editDoc}.jpg`,
                );
            ...
            try {
                firestore().collection('Capitulo3').doc(editDoc).update({
                    titulo,
                    descripcion,
                    imagen: imageUrl,
                    QR: imageQUrl,
