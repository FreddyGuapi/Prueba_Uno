...
const OptionTem1 = ({ navigation }) => {
    ...
    async function loadRTDAta() {
        const suscriber = firestore().collection('Capitulo1').onSnapshot(querySnapshot => {
            const temas = []
        ...
        Alert.alert(
            ...
            [
                ...
                },
                {
                    text: "Si",
                    onPress: () => {
                        firestore().collection('Capitulo1').doc(item.key).delete().then(() => {
                            Alert.alert(
                                "Eliminación Exitosa",
        ...
        if (titulo == '' || descripcion == '') {
            ...
            if (imagen != '') {
                const reference = storage().ref(
                    `/Capitulos/Capitulo1/imagenes/${editDoc}.jpg`,
                );
            ...
            if (QR != '') {
                const reference = storage().ref(
                    `/Capitulos/Capitulo1/QR/${editDoc}.jpg`,
                );
            ...
            try {
                firestore().collection('Capitulo1').doc(editDoc).update({
                    titulo,
                    descripcion,
                    imagen: imageUrl,
                    QR: imageQUrl,
                ...
                ToastAndroid.show('Tema modificado', ToastAndroid.SHORT);
                setTitulo('')
                setImagen('')
                setDescripcion('')
                setQR('')
