...
import { getTemas5_6 } from '../../../utils/database';

const TemasUser5_6 = ({ navigation }) => {
  const [search, setsearch] = useState('')
  const [allQuizzes, setAllQuizzes] = useState([]);
  const [refreshing, setRefreshing] = useState(false);
  const getAllQuizzes = async () => {
    setRefreshing(true);
    const quizzes = await getTemas5_6();
    let tempQuizzes = [];
  ...
  return (
    <SafeAreaView
      ...
      <TextInput
        ...
        style={{ backgroundColor: COLORS.white, borderColor: COLORS.black, marginHorizontal: 10, marginHorizontal: 10, borderRadius: 10, marginTop: 10 }}
      />
      {/* Listado de Temas */}
      <FlatList
        data={allQuizzes}
        ...
        renderItem={({ item: quiz }) => (
          <View
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                navigation.navigate('InfoTema5_6', {
                  quizId: quiz.id,
                });
              }}>
