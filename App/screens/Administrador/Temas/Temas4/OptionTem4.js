...
const OptionTem4 = ({ navigation }) => {
    ...
    async function loadRTDAta() {
        const suscriber = firestore().collection('Capitulo4').onSnapshot(querySnapshot => {
            const temas = []
        ...
        Alert.alert(
            ...
            [
                ...
                },
                {
                    text: "Si",
                    onPress: () => {
                        firestore().collection('Capitulo4').doc(item.key).delete().then(() => {
                            Alert.alert(
                                "Eliminación Exitosa",
    ...
    const editarTema = async () => {
        if (titulo == '' || descripcion == '') {
            ...
            try {
                firestore().collection('Capitulo4').doc(editDoc).update({
                    titulo,
                    descripcion,
                    imagen: imageUrl,
                    QR: imageQUrl,
